...
| `dynamic_threshold_service.py` | Computes adaptive DT values per bin using fill-rate + capacity formula. |
...
## ♻️ Dynamic threshold (DT)
We now compute a per-bin dispatch threshold using the time-based formula provided for Dec 16:

1. **Core math**: \( \text{DT} = 100 \times \left(1 - \frac{\text{fillRate} \times T_{min}}{\text{capacity}}\right) \), with \(T_{min}=24\) hours.
2. **Safeguards**:
  - If `fillRate <= 0` or `< 0.01 L/h`, DT defaults to **95 %** (bin can safely wait).
  - Clamp output between **50 %** (don’t collect too early) and **95 %** (leave 5 % headroom).
3. **Storage**: Every bin carries `dynamic_threshold` and the frontend now displays `DT` instead of the static `threshold`.
4. **Dispatch usage**: All bin classification and urgency checks rely on `dynamic_threshold` first, falling back to the legacy `threshold` only when DT is unavailable.

This keeps slow bins from triggering premature runs while forcing fast-fill bins to dispatch as soon as practical.
